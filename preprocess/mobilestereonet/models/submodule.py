...
def groupwise_correlation(fea1, fea2, num_groups):
    # 获取 fea1 和 fea2 的尺寸
    B1, C1, H1, W1 = fea1.shape
    B2, C2, H2, W2 = fea2.shape

    # 确保批次大小和通道数一致
    assert B1 == B2, f"Batch size mismatch: {B1} vs {B2}"
    assert C1 == C2, f"Channel mismatch: {C1} vs {C2}"
    assert C1 % num_groups == 0, f"Channels {C1} must be divisible by num_groups {num_groups}"
    ...
    # 计算分组相关性
    channels_per_group = C1 // num_groups
    cost = (fea1 * fea2).view([B1, num_groups, channels_per_group, H, W]).mean(dim=2)

    # 确保输出尺寸正确
    assert cost.shape == (B1, num_groups, H, W), f"Output shape mismatch: {cost.shape} vs {(B1, num_groups, H, W)}"
    return cost

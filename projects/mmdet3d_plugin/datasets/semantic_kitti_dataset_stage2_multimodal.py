...
class SemanticKittiDatasetStage2_multimodal(Dataset):
    """
    Advised by Hao:
    ...
    def load_scans(self):
        ...
        for sequence in self.sequences:
            ...
            #     self.data_root, "dataset", "sequences_" + self.depthmodel + "_sweep" + self.nsweep, sequence, "queries",
            #                                "*." + self.query_tag
            # )
            glob_path = os.path.join(
                self.data_root, "lidar_queries", sequence, "queries",
                                           "*." + self.query_tag
            )
    ...
    def get_data_info(self, index):
        ...
        # 后续需要注释掉
        frame_id, others = frame_id.split(".")
    ...
    def get_input_info(self, sequence, frame_id):
        ...
        if self.event_input is True:
            event_frame_path = os.path.join(
                self.data_root, "dataset", "sequences", sequence, "image_0", frame_id + ".npy"
        ...
        for i in self.target_frames:
            ...
            if self.event_input is True:
                event_frame_path = os.path.join(
                    self.data_root, "dataset", "sequences", sequence, "image_0", target_id + ".npy"
                )  # 4 channel (pos_cnt, neg_cnt, pos_near, neg_near) H*W*4    3-sigma归一化到(0, 255)
                event_frame = np.load(event_frame_path)
                assert self.color_jitter is None  # Baseline没有增强，事件帧增强不等同图像增强
                event_frame = cv2.resize(event_frame, (img_w, img_h))  # resize事件帧到一样大小

...
class VoxFormerHead_2(nn.Module):
    def __init__(
        self,
        *args,
        bev_h,
        bev_w,
        bev_z,
        cross_transformer_img,
        self_transformer_img,
        positional_encoding_img,
        cross_transformer_ev,
        self_transformer_ev,
        positional_encoding_ev,
        mm_fusion2,
        embed_dims,
        CE_ssc_loss=True,
        geo_scal_loss=True,
        ...
        **kwargs
        ...
        self.positional_encoding_img = build_positional_encoding(positional_encoding_img)
        self.cross_transformer_img = build_transformer(cross_transformer_img)
        self.self_transformer_img = build_transformer(self_transformer_img)
        self.positional_encoding_ev = build_positional_encoding(positional_encoding_ev)
        self.cross_transformer_ev = build_transformer(cross_transformer_ev)
        self.self_transformer_ev = build_transformer(self_transformer_ev)
        if mm_fusion2 is 'cat':
            self.header = Header(self.n_classes, nn.BatchNorm3d, feature=self.embed_dims*2)
        else:
    ...
    def forward(self, mlvl_feats0, img_metas, target):
        """Forward function.
        Args:
        ...
        for combined_feat in mlvl_feats0:
            ...
            half_C = C_combined // 2  # 计算一半的通道数

            # 前一半通道
            mlvl_feat = combined_feat[:,:, :half_C, :, :]
            mlvl_feats.append(mlvl_feat)

            # 后一半通道
            mlvl_feat_ev= combined_feat[:,:, half_C:, :, :]
            mlvl_feats_ev.append(mlvl_feat_ev)

        # img_feats_combined_list 和 ev_feats_combined_list 分别包含所有特征图的前一半和后一半通道

        bs, num_cam, _, _, _ = mlvl_feats[0].shape
        dtype = mlvl_feats[0].dtype
        bev_queries = self.bev_embed.weight.to(dtype) #[128*128*16, dim]
        dtype_ev = mlvl_feats_ev[0].dtype
        bev_queries_ev = self.bev_embed.weight.to(dtype_ev)  # [128*128*16, dim]
        # Generate bev postional embeddings for cross and self attention
        bev_pos_cross_attn = self.positional_encoding_img(torch.zeros((bs, 512, 512), device=bev_queries.device).to(dtype)).to(dtype) # [1, dim, 128*4, 128*4]
        bev_pos_self_attn = self.positional_encoding_img(torch.zeros((bs, 512, 512), device=bev_queries.device).to(dtype)).to(dtype) # [1, dim, 128*4, 128*4]

        ############
        bev_pos_cross_attn_ev = self.positional_encoding_ev(torch.zeros((bs, 512, 512), device=bev_queries_ev.device).to(dtype)).to(dtype) # [1, dim, 128*4, 128*4]
        bev_pos_self_attn_ev = self.positional_encoding_ev(torch.zeros((bs, 512, 512), device=bev_queries_ev.device).to(dtype)).to(dtype) # [1, dim, 128*4, 128*4]
        ...
        seed_feats = self.cross_transformer_img.get_vox_features(
            mlvl_feats,
            bev_queries,
            self.bev_h,
            self.bev_w,
        ...
        seed_feats_ev = self.cross_transformer_ev.get_vox_features(
            mlvl_feats_ev,
            bev_queries_ev,
            self.bev_h,
            self.bev_w,
        ...
        vox_feats_ev = torch.empty((self.bev_h, self.bev_w, self.bev_z, self.embed_dims), device=bev_queries_ev.device)
        vox_feats_flatten_ev = vox_feats_ev.reshape(-1, self.embed_dims)
        vox_feats_flatten_ev[vox_coords[unmasked_idx[0], 3], :] = seed_feats_ev[0]
        vox_feats_flatten_ev[vox_coords[masked_idx[0], 3], :] = self.mask_embed.weight.view(1, self.embed_dims).expand(masked_idx.shape[1], self.embed_dims).to(dtype)
        # Diffuse voxel features by deformable self attention
        vox_feats_diff = self.self_transformer_img.diffuse_vox_features(
            mlvl_feats,
            vox_feats_flatten,
            512,
            512,
        ...
        vox_feats_diff_ev = self.self_transformer_ev.diffuse_vox_features(
            mlvl_feats_ev,
            vox_feats_flatten_ev,
            512,
            512,
            ref_3d=ref_3d,
            vox_coords=vox_coords,
        ...
        vox_feats_diff_ev = vox_feats_diff_ev.reshape(self.bev_h, self.bev_w, self.bev_z, self.embed_dims)
        img_voxel = vox_feats_diff.permute(3, 0, 1, 2).unsqueeze(0)
        ev_voxel = vox_feats_diff_ev.permute(3, 0, 1, 2).unsqueeze(0)
        if self.mm_fusion2 is 'cat':
            voxel_feats = torch.cat((img_voxel, ev_voxel), dim=1)
        if self.mm_fusion2 is 'add':
            voxel_feats = img_voxel+ev_voxel
        input_dict = {
            "x3d": voxel_feats,
        }

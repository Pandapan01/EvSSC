...
class VoxFormerMultiModal_deep_lidar(MVXTwoStageDetector):
    """
    多模态输入版本的VoxFormer模型
    New args:
    ...
    def __init__(self,
        ...
        super(VoxFormerMultiModal_deep_lidar,
    ...
    def extract_img_feat(self, img, img_metas, len_queue=None):
        ...
        if img is not None:
            ...
            if self.multimodal_backbone:
                ...
                img_lidar_feats = self.lidar_backbone(img_lidar)
                residual1 = img_img_feats[0]
                residual2 = img_mm_feats[0]
                residual3 = img_lidar_feats[0]
                if self.mm_fusion is 'cat_1x1':
                    ...
                    img_feats = (torch.cat((img_img_feats[0], img_mm_feats[0],img_lidar_feats[0]), dim=1),)
        ...
        if self.with_img_neck:
            ...
            lidar_feats = (residual3,)
            lidar_feats = self.lidar_neck(lidar_feats)

        img_feats_reshaped = []
        ev_feats_reshaped = []
        lidar_feats_reshaped = []
        ...
        for lidar_feat in lidar_feats:
            BN, C, H, W = lidar_feat.size()
            if len_queue is not None:
                lidar_feats_reshaped.append(lidar_feat.view(int(B / len_queue), len_queue, int(BN / B), C, H, W))
            else:
                lidar_feats_reshaped.append(lidar_feat.view(B, int(BN / B), C, H, W))

        combined_feats = []
        for img_feat, ev_feat,lidar_feat in zip(img_feats_reshaped, ev_feats_reshaped,lidar_feats_reshaped):
            # 确保形状相同
            assert img_feat.size() == ev_feat.size() == lidar_feat.size(), "Shape mismatch between img_feat and ev_feat"
            combined_feat = torch.cat((img_feat, ev_feat,lidar_feat), dim=2)  # 在通道维度拼接

...
from projects.mmdet3d_plugin.voxformer.utils.ssc_loss import sem_scal_loss, CE_ssc_loss, KL_sep, geo_scal_loss, \
    BCE_ssc_loss
...
class LMSCNet_SS(MVXTwoStageDetector):
    def __init__(self,
        ...
        self.class_frequencies_level1 = np.array([8.81908307e08, 3.2449965e07])
    ...
    def pack(self, array):
        ...
        # yapf: disable
        compressed = array[::8] << 7 | array[1::8] << 6 | array[2::8] << 5 | array[3::8] << 4 | array[
                                                                                                4::8] << 3 | array[
                                                                                                             5::8] << 2 | array[
                                                                                                                          6::8] << 1 | array[
                                                                                                                                       7::8]
    ...
    def foward_training(self,
        ...
        depth = torch.from_numpy(img_metas[0]["pseudo_pc"]).reshape(256, 256, 32).unsqueeze(0).float()  # [1, 256, 256, 32]
    ...
    def foward_test(self,
        ...
        depth = torch.from_numpy(img_metas[0]["pseudo_pc"]).reshape(256, 256, 32).unsqueeze(0).float()
        # [1, 256, 256, 32]
        ...
        #         os.path.join("/mnt1/gsw/Dsec_SSC/dataset/sequences_msnet3d_sweep10", img_metas[0]['sequence_id'], 'queries')):
        #     os.makedirs(
        #         os.path.join("/mnt1/gsw/Dsec_SSC/dataset/sequences_msnet3d_sweep10", img_metas[0]['sequence_id'], 'queries'))
        # save_query_path = os.path.join("/mnt1/gsw/Dsec_SSC/dataset/sequences_msnet3d_sweep10", img_metas[0]['sequence_id'],
        #                                'queries', frame_id + ".query_iou4151")
        if not os.path.exists(
                os.path.join("/mnt/sto/gsw/voxformer/kitti/", 'lidar_queries',img_metas[0]['sequence_id'], 'queries')):
            os.makedirs(
                os.path.join("/mnt/sto/gsw/voxformer/kitti/", 'lidar_queries',img_metas[0]['sequence_id'], 'queries'))
        save_query_path = os.path.join("/mnt/sto/gsw/voxformer/kitti/",'lidar_queries', img_metas[0]['sequence_id'],
                                       'queries', frame_id + ".query_iou62.89epoch22")

...
class PerceptionTransformer2deep(BaseModule):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(PerceptionTransformer2deep, self).__init__(**kwargs)
    ...
    def get_vox_features(
        ...
        spatial_shapes = []
        feat_flatten_ev = []
        for lvl, (feat, feat_ev) in enumerate(zip(mlvl_feats, mlvl_feats_ev)):
            bs, num_cam, c, h, w = feat.shape
            spatial_shape = (h, w)
            feat = feat.squeeze(0)
            ...
            feat_com = self.multimodal_fusion(feat_ev,feat)
